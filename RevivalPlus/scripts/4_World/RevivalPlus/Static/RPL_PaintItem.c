class RPL_PaintItem
{
    ...
    static void Paint(ItemBase item_tool, ItemBase item_target, string base_name, PlayerBase player, float specialty_weight = 0)
    {
        ...
        string new_class_name = "RPL_" + base_name + "_" + spray_color;
        PaintItem.SwitchItems(item_target, new_class_name, player);
    }

    static void RemovePaint(ItemBase item_target, string base_name, PlayerBase player, float specialty_weight = 0)
    {
        PaintItem.SwitchItems(item_target, base_name, player);
    }
